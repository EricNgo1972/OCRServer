#!/usr/bin/env bash
# Run this once in WSL (Ubuntu/Debian) so the OCR Server can load OpenCvSharp and Tesseract natives.
...
if ! command -v apt-get >/dev/null 2>&1; then
  echo "This script expects apt-get (Ubuntu/Debian). For other distros, install: libtesseract, leptonica, libgdiplus."
  exit 1
fi
sudo apt-get update
